...
注入沿逻辑组件父链解析，可以跨 open/closed Shadow Root；Teleport 到 `body` 后也不会丢失
原 Provider 或 App 上下文。嵌套 Provider 以最近者优先，后插入的 Custom Element 会读取
连接时可见的值。需要动态更新时应 provide Ref 或响应式对象：
...
暴露 `focus()`、`blur()` 等 HTMLElement 已有成员时，子组件需要通过
`defineExpose(api, { overrideNative: ["focus"] })` 明确声明覆盖。
